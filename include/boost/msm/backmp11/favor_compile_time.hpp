...
struct compile_policy_impl<favor_compile_time>
{
    ...
    static const std::unordered_set<std::type_index>& get_deferred_event_type_indices()
    {
        ...
        mp11::mp_for_each<deferred_event_identities>(
            [](auto event_identity)
            {
                ...
                type_indices.emplace(to_type_index<Event>());
            }
        );
        return type_indices;
    }
    ...
    template <typename StateMachine>
    static bool to_be_deferred(StateMachine& sm, const any_event& event)
    {
        bool result = false;
        auto visitor = [&result, &event](auto& state) {
            using State = std::decay_t<decltype(state)>;
            auto& set = get_deferred_event_type_indices<State>();
            result |= (set.find(event.type()) != set.end());
        };
    ...
    private:
        ...
        dispatch_table()
        {
            ...
            using filtered_states = mp11::mp_copy_if<state_set, has_back_end_tag>;
            mp11::mp_for_each<mp11::mp_transform<mp11::mp_identity, filtered_states>>(
                [this](auto state_identity)
                {
                    ...
                    static constexpr int state_id = Fsm::template get_state_id<State>();
                    m_state_dispatch_tables[state_id + 1].template init_call_submachine<State>();
                });
        }
