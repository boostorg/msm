...
class state_machine_base : public FrontEnd
{
  ...
  private:
    ...
    {
        std::function<process_result()> process_event_function;
        std::function<bool()> to_be_deferred_function;
    ...
    bool to_be_deferred(const Event &event) const
    {
        return compile_policy_impl::to_be_deferred(
    ...
    void try_process_deferred_events()
    {
        if constexpr (deferred_events_member::value)
        {
            ...
            do
            {
                ...
                if (it->to_be_deferred_function())
                {
                    it = std::next(it);
                }
                else
                {
                    const process_result result = it->process_event_function();
