...
=== Reflection API (serializers)

The `state_machine` allows access to its private members for reflection use cases with a free function:
...
// - (T&, State&, ReflectFunction&&) for submachines and states with reflection
// - (T&, State&) for states without reflection
// - (T&, const char* /*key*/, Member&) for all other members
template<typename FrontEnd, typename Config, typename Derived, typename T, typename F>
void reflect(detail::state_machine_base<FrontEnd, Config, Derived>& sm, T& obj, F&& f)
{
    detail::reflect(sm, obj, std::forward<F>(f));
}
```

The reflection API can be used to serialize a state machine. You can find example integration code for the most widely used serialization libraries link:TODO[here].

[IMPORTANT]
====
The called overloads of the functor F are stable, but do not rely on any assumptions about the call orders or argument details.
The reflection API access private members of the state machine, these are subject to changes and there is no guarantee on
the memory layout of the `state_machine` class.
====
