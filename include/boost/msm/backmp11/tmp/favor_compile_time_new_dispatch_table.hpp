...
struct new_dispatch_table <Fsm, Stt, favor_compile_time>
{
 private:
    // Compute the maximum state & event value in the sm so we know how big
    // to make the table
    ...
    BOOST_STATIC_CONSTANT(int, max_event = (mp11::mp_size<event_set>::value));
    ...
    struct init_helper
    {
        ...
        // This is a row of these function pointers.
        ...
        init_helper(chain_row* all_entries_) : all_entries(all_entries_) {}

        // initialize the row for a given Event and Fsm
        template<typename Event>
        void operator()(const Event&, typename enable_if<mp11::mp_set_contains<event_set, Event>>::type* = 0)
        {
            auto entries = &all_entries[get_event_id<Stt, Event>::value];

            // Fill in cells for matching transitions
            typedef mp11::mp_transform<
                event_helper<Event>::template preprocess_row,
                get_rows_of_event<Stt, Event>
                > preprocessed_rows;
            cell_initializer::init(
                entries,
                get_init_cells<typename event_helper<Event>::cell, preprocessed_rows>(),
            ...
            typedef mp11::mp_copy_if<
                ...
                event_helper<Event>::template state_filter_predicate
                > filtered_states;
            typedef mp11::mp_transform<
                event_helper<Event>::template preprocess_state,
                filtered_states
                > preprocessed_states;
            cell_initializer::init(
                entries,
                get_init_cells<typename event_helper<Event>::cell, preprocessed_states>(),
        ...
        template<typename Event>
        void operator()(const Event&, typename disable_if<mp11::mp_set_contains<event_set, Event>>::type* = 0)
        {
            auto entries = &all_entries[get_event_id<Stt, Event>::value];
            // Initialize cells that defer an event or call a submachine.
            typedef mp11::mp_copy_if<
        ...
        chain_row* all_entries;
    };

 public:
    // initialize the dispatch table for a given Fsm
    new_dispatch_table()
    {
        mp11::mp_for_each<event_set>(init_helper{reinterpret_cast<chain_row*>(entries)});
    }

    // The singleton instance.
    static const new_dispatch_table& instance() {
        static new_dispatch_table table;
        return table;
    }

 public: // data members
     chain_row entries[max_event][max_state+1];
};
