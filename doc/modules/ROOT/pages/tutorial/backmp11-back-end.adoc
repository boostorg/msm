...
| Removal of APIs to enqueue events and process queued events
| 1.91 / 1.92
a| The event containers for queued events and deferred events have been merged into a single event pool. The APIs related to queued events are obsolete.

*Change:* The APIs `enqueue_event`, `process_queued_events`, and `process_single_queued_event` will be removed.

Occurrences of the old APIs can be replaced as follows:

- to process an event as soon as possible, use `process_event`
- to process an event in the next RTC sequence, use `defer_event`
- to process one or more events from the event pool, use `process_event_pool` +
  (this API allows an optional limit for the max. no. of events to be processed and additionally returns the no. of actually processed events)
...
class state_machine_adapter
{
    ...
    {
        return is_flag_active<Flag, flag_and>();
    }
