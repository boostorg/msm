...
It provides a _basic exception guarantee_: The processing aborts mid-execution, but the state machine remains in a valid state. If the processing step involves a state switch, the active state switches to the target state as soon as the source state's exit action has completed. If you need to know exact details about the aborted state machine activities during event processing, take a look at the xref:#_run_to_completion[run-to-completion algorithm].

You can enqueue an event for consecutive processing in an action with `void enqueue_event(const Event&)`. The enqueued event will be processed immediately after the current event is done processing.
...
| 1.92 / 1.93
a| The UML specification describes the exact point in time when an active state switch in a transition is expected to happen. The current setting `active_state_switch_policy` in the front-end allows to select unspecified values. Its default value "after target entry" is not UML compliant.

*Change:* The active state switch policy is set to "after source exit" in compliance to the UML specification.
