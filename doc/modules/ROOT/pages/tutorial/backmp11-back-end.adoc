...
=== Reflection API and serialization examples

The `state_machine` allows access to its private members with a free function:
...
You can use the reflection API for introspection use cases, the most prominent one being serialization. Two comprehensive integration examples for serialization libraries are at your disposal to be taken over and adjusted to your serialization needs:

- link:TODO[Boost.Serialization]
- link:TODO[nlohmann json]


[IMPORTANT]
====
The required API of the Visitor is stable. However, do not rely on assumptions about call orders or cardinalities. The reflection API exposes private members of the state machine, which are subject to changes. There is no guarantee about the memory layout of the `state_machine` class.
====
