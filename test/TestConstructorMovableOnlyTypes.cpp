...
namespace
{
    ...
    struct bistable_switch_ : public msm::front::state_machine_def<bistable_switch_>
    {
        ...
        struct Off : public msm::front::state<> 
        {
            ...
            void on_entry(Event const&, FSM& fsm) { std::cout << "entering: Off" << std::endl; }
            template <typename Event, typename FSM>
            void on_exit(Event const&, FSM&) { std::cout << "leaving: Off" << std::endl; }
        };

        struct On : public msm::front::state<> 
        {
            template <typename Event, typename FSM>
            void on_entry(Event const&, FSM& fsm) { std::cout << "entering: On" << std::endl; }
            template <typename Event, typename FSM>
            void on_exit(Event const&, FSM&) { std::cout << "leaving: On" << std::endl; }
        };

        // the initial state of the player SM. Must be defined
        typedef Off initial_state;

        void turn_on(ev_toggle const&) { bulp_->current = 11; std::cout << "bistable_switch::turn_on" << std::endl; }
        void turn_off(ev_toggle const&) { bulp_->current = 9; std::cout << "bistable_switch::turn_off" << std::endl; }
