...
class boost_serializer
{
  public:
    ...
    {
        serialize(std::forward<FrontEnd>(front_end));
    }
    ...
    {
        serialize(std::forward<State>(state));
    }
    ...
    void serialize(T& arg)
    {
        ...
        {
            auto& bytes =
                reinterpret_cast<std::array<std::byte, sizeof(T)>&>(arg);
            m_archive & bytes;
        }
        else
        {
            static_assert(std::is_empty_v<T>,
                          "T must have reflection or a serialize member");
        }
    }
